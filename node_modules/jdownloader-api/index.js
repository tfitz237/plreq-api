...
exports.addLinks = (links, deviceId, name) => {
  const params = `{"priority":"DEFAULT","links":"${links}","autostart":true, "packageName":"${name}"}`;
  return new Promise((resolve, rejected) => {
...
exports.cleanUp = (deviceId, packageIds, action, selectionType, mode) => {

  action = action || "DELETE_FINISHED";
  mode = mode || "REMOVE_LINKS_AND_DELETE_FILES";
  selectionType = selectionType || "SELECTED";
  const params = [[], packageIds || [], action, mode, selectionType]; 
  return new Promise((resolve, rejected) => {
    callAction('/downloadsV2/cleanup', deviceId, params)
      .then((val) => {
        resolve(val);
      }).catch((error) => {
        error.error = decrypt(error.error, __deviceEncryptionToken);
        rejected(error);
      });
  });
};
